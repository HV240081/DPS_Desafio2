...
const PantallaEditarCita = ({ route, navigation }) => {
  const { cita } = route.params;
  const [clientName, setClientName] = useState(cita.clientName);
  const [vehicleModel, setVehicleModel] = useState(cita.vehicleModel);
  const [date, setDate] = useState(new Date(cita.date));
  const [selectedHour, setSelectedHour] = useState(parseInt(cita.time.split(':')[0]));
  const [selectedMinute, setSelectedMinute] = useState(parseInt(cita.time.split(':')[1]));
  const [description, setDescription] = useState(cita.description || '');
  ...
  const handleSubmit = async () => {
    ...
    const isDuplicate = citas.some(
      (existingCita) =>
        existingCita.id !== updatedAppointment.id &&
        existingCita.date === formattedDate &&
        existingCita.time === formattedTime &&
        existingCita.vehicleModel === vehicleModel
    );
    if (isDuplicate) {
    ...
    const updatedCitas = citas.map((existingCita) =>
      existingCita.id === updatedAppointment.id ? updatedAppointment : existingCita
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title={new Date(date).toLocaleDateString()} onPress={() => setShowDatePicker(true)} />
      {showDatePicker && (
        <DateTimePicker
          value={date}
          mode="date"
